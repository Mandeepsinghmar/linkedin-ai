...
const ChatBox = ({ setIsModalOpen }) => {
  ...
  const handleInsert = () => {
    ...
    // Logic to insert generatedText into the message input field
    const inputField = document.querySelector(".msg-form__contenteditable > p")
    if (inputField) {
  ...
  return (
    <div
      className="bg-[#F9FAFB] text-2xl w-[500px] p-6 rounded-md"
      ...
      <div className="flex-1 py-4 overflow-y-auto">
        {messages.map((message, index) => (
          <div
            ...
            <div
              className={` my-2 rounded-lg max-w-[368px] break-words p-4 ${message.sender === "user" ? "bg-[#DFE1E7] text-[#666D80]" : " bg-[#DBEAFE] text-[#666D80]"}`}>
              {message.text}
            </div>
          </div>
        ))}
      ...
      <div className="flex justify-end gap-2">
        {generatedText && (
          <div
            ...
            className="flex cursor-pointer gap-2 items-center border border-[#666D80] text-[#666D80] px-6 py-3 rounded-md">
            <svg
              width="12"
              height="14"
              viewBox="0 0 15 17"
        ...
        {generatedText ? (
          <div className="bg-[#3B82F6] flex cursor-pointer gap-2 items-center text-white px-6 py-3 rounded-md ">
            <svg
              width="12"
              height="18"
              viewBox="0 0 20 29"
          ...
          <div
            ...
            className="bg-[#3B82F6] flex cursor-pointer gap-2 items-center text-white px-6 py-3 rounded-md ">
            <svg
              width="14"
              height="14"
              viewBox="0 0 25 25"
